...
def create_post():
    ...
    if request.method == 'POST':
        ...
        # Check if the user's total contribution >= 50
        ...
        # Set post status based on contribution
...
def upvote(post_id):
    if 'username' not in session:
        return redirect(url_for('login'))
    posts.update_one({'_id': ObjectId(post_id)}, {'$inc': {'upvotes': 1}})
...
def downvote(post_id):
    if 'username' not in session:
        return redirect(url_for('login'))
    posts.update_one({'_id': ObjectId(post_id)}, {'$inc': {'downvotes': 1}})
...
#     user_posts = posts.find({'username': username})
    
#     # Calculate total contribution
#     total_contribution = sum(post.get('upvotes', 0) - post.get('downvotes', 0) for post in user_posts)
    
#     return render_template('profile.html', user=user, posts=user_posts, total_contribution=total_contribution)
...
def assign_moderator(username):
    ...
    return redirect(url_for('dashboard_profiles'))
